...
class BACON(MFNBase):
    def __init__(self,
        ...
        # Output color and displacement layers 
        color_layers = []
        for _ in range(colordepth):
            color_layers.append(nn.Linear(hidden_size, hidden_size))
            color_layers.append(nn.ReLU())
        color_layers.append(nn.Linear(hidden_size, 3))
        self.mlp_rgb = nn.ModuleList(color_layers)

        normal_layers = []
        for _ in range(normdepth):
            normal_layers.append(nn.Linear(hidden_size, hidden_size))
            normal_layers.append(nn.ReLU())
        normal_layers.append(nn.Linear(hidden_size, 1))
    ...
    def forward_mfn(self, coords):
        ...
        # out = self.output_linear(out) # diabled for text2mesh

        return out

    def forward(self, coords):
        out = self.forward_mfn(coords)

        colors = self.mlp_rgb[0](out)
        for layer in self.mlp_rgb[1:]:
            colors = layer(colors)
        displ = self.mlp_normal[0](out)
        ...
        if self.clamp == "tanh":
            ...
            colors = torch.clamp(colors, 0, 1) # TODO: clamp to [-0.5, 0.5]?
        else:
            raise ValueError(f"clamp must be 'tanh' or 'clamp', was {self.clamp}")
        if self.normclamp == "tanh":
            ...
            raise ValueError(f"normclamp must be 'tanh' or 'clamp', was {self.normclamp}")

        return colors, displ

    def shrink_last_layer_weights(self, shrink_factor=0.0001):
        self.mlp_rgb[-1].weight.data.mul_(shrink_factor)
        self.mlp_rgb[-1].bias.data.mul_(shrink_factor)
        self.mlp_normal[-1].weight.data.mul_(shrink_factor)
        self.mlp_normal[-1].bias.data.mul_(shrink_factor)

    def init_weights(self):
        for layer in self.mlp_rgb:
            layer.apply(mfn_weights_init)
        for layer in self.mlp_normal:
            layer.apply(mfn_weights_init)
        self.shrink_last_layer_weights(shrink_factor=0.0001)
        print('BACON weights initialized, last layer weights shrunk.')

class MultiscaleBACON(nn.Module):
    def __init__(self,
                 in_size, hidden_size, hidden_layers=3,
                 bias=True, frequency=(128, 128),
                 quantization_interval=2*np.pi, centered=True,
                 is_sdf=False, input_scales=None,
                 output_layers=None, reuse_filters=False,
                 normratio=0.1, clamp=None, normclamp=None,
                 colordepth=2, normdepth=2):
        super().__init__()

        self.linear = nn.ModuleList(
            [nn.Linear(hidden_size, hidden_size, bias) for _ in range(hidden_layers)]
        )
        self.linear.apply(mfn_weights_init)
        ...
        self.centered = centered
        self.is_sdf = is_sdf
        assert is_sdf == False, "SDF not implemented yet for text2mesh"
        self.frequency = frequency
        self.output_layers = output_layers
        self.reuse_filters = reuse_filters
        assert reuse_filters == False, "reuse_filters not implemented yet for text2mesh"
        ...
        def get_color_mlp():
            ...
            for _ in range(colordepth):
                ...
                color_layers.append(nn.ReLU())
            color_layers.append(nn.Linear(hidden_size, 3))
            mlp_rgb = nn.Sequential(*color_layers)
            return mlp_rgb
        
        self.mlp_rgb_list = nn.ModuleList([get_color_mlp() for i in range(hidden_layers)])
        ...
        def get_normal_mlp():
            ...
            for _ in range(normdepth):
                ...
                normal_layers.append(nn.ReLU())
            normal_layers.append(nn.Linear(hidden_size, 1))
            mlp_normal = nn.Sequential(*normal_layers)
            return mlp_normal

        self.mlp_normal_list = nn.ModuleList([get_normal_mlp() for i in range(hidden_layers)])

        print(self)

    def forward_bacon(self, coords, specified_layer=0):
        # no layer reuse
        out = self.filters[0](coords)
        for i in range(1, specified_layer+1+1):
            out = self.filters[i](coords) * self.linear[i - 1](out)

        return out

    def forward(self, coords, specified_layer=None):
        if specified_layer is None:
            specified_layer = self.hidden_layers - 1
        assert 0 <= specified_layer < self.hidden_layers, "specified_layer must be in [0, hidden_layers)"

        out = self.forward_bacon(coords, specified_layer=specified_layer)

        mlp_rgb = self.mlp_rgb_list[specified_layer]
        colors = mlp_rgb[0](out)
        for layer in mlp_rgb[1:]:
            colors = layer(colors)

        mlp_normal = self.mlp_normal_list[specified_layer]
        displ = mlp_normal[0](out)
        for layer in mlp_normal[1:]:
            displ = layer(displ)

        if self.clamp == "tanh":
            colors = torch.tanh(colors) * 0.5
        else:
            raise ValueError(f"clamp must be 'tanh' was {self.clamp}")
        
        if self.normclamp == "tanh":
            displ = torch.tanh(displ) * self.normratio
        else:
            raise ValueError(f"normclamp must be 'tanh' was {self.normclamp}")

        return colors, displ

    def shrink_last_layer_weights(self, shrink_factor=0.0001):
        for mlp_rgb in self.mlp_rgb_list:
            mlp_rgb[-1].weight.data.mul_(shrink_factor)
            mlp_rgb[-1].bias.data.mul_(shrink_factor)
        for mlp_normal in self.mlp_normal_list:
            mlp_normal[-1].weight.data.mul_(shrink_factor)
            mlp_normal[-1].bias.data.mul_(shrink_factor)

    def init_weights(self):
        for mlp_rgb in self.mlp_rgb_list:
            for layer in mlp_rgb:
                layer.apply(mfn_weights_init)
        for mlp_normal in self.mlp_normal_list:
            for layer in mlp_normal:
                layer.apply(mfn_weights_init)
        self.shrink_last_layer_weights(shrink_factor=0.0001)
        print('MultiscaleBACON weights initialized, last layer weights shrunk.')

    def get_params_contributing_to_layer(self, specified_layer):
        assert 0 <= specified_layer < self.hidden_layers, "specified_layer must be in [0, hidden_layers)"
        params = []
        for i in range(specified_layer+1):
            params += list(self.filters[i].parameters())
        for i in range(specified_layer):
            params += list(self.linear[i].parameters())
        params += list(self.mlp_rgb_list[specified_layer].parameters())
        params += list(self.mlp_normal_list[specified_layer].parameters())
        return params

    def get_params_for_layer(self, specified_layer):
        assert 0 <= specified_layer < self.hidden_layers, "specified_layer must be in [0, hidden_layers)"
        params = []
        if specified_layer == 0:
            params += list(self.filters[0].parameters())
        params += list(self.filters[specified_layer+1].parameters())
        params += list(self.linear[specified_layer].parameters())
        params += list(self.mlp_rgb_list[specified_layer].parameters())
        params += list(self.mlp_normal_list[specified_layer].parameters())
        return params
...
class PositionalEncoding(nn.Module):
    def __init__(self, num_encoding_functions=6, include_input=True, log_sampling=True, normalize=False,
        ...
        if self.gaussian_pe:
            ...
            # and so that it is converted using .to('cpu'). Doesn't need to be trained though
