...
from prompt_templates import no_template, imagenet_templates, avatar_template
...
# write a function that creates a gif from a list of tensor images
def make_gif(images, fname, duration=1, true_image=False):
    ...
    for i in range(images.shape[0]):
        ...
        # turn the image from [0,1] to [0,255] and unit8 numpy
        image = np.uint8(image * 255)
        imgs.append(Image.fromarray(image))
    # imgs = [Image.fromarray(img) for img in imgs]
    # duration is the number of milliseconds between frames; this is 40 frames per second
    imgs[0].save(fname, save_all=True, append_images=imgs[1:], duration=duration, loop=0)

    return fname

def diversify_batch_trans(aug_transform):
    def divers_batch_trans(x):
        res = torch.stack([aug_transform(x_) for x_ in x])
        return res
    return divers_batch_trans

def add_color_to_prompt(prompt: str, back_color: float = 1.0):
    map_color = {0.0: 'black', 1.0: 'white', 0.5: 'gray'}
    prompt = prompt + " on " + map_color[back_color] + " background"
    return prompt

def add_background_to_prompt(prompt, background_type = 'white'):
    map_color = {'red': 'red background.', 'green': 'green screen.', 
                 'blue': 'blue background.', 'white': 'white background.',
                 'black': 'black background.'}
    if background_type in map_color:
        prompt = prompt.strip('.').strip() + " on a " + map_color[background_type] 
    else:
        prompt = prompt
    return prompt

def parse_clip_model_names(model_names, coeff_models_deviate):
        """
        Parse the model names from the CLI. Returns the clip model names, their 
        ensamble weights, and devices to load them on.
        """
        model_name_list = [x.split(':')[0] for x in model_names.split(',')]
        model_device_list = ['cuda:0' if len(x.split(':'))==1 else 'cuda:'+x.split(':')[1] for x in model_names.split(',')]
            
        # make a list of the coeff_model which sums to one and the largest elemnet is 1 - coeff_models_deviate and linearly spaced
        if len(model_name_list) == 1:
            coeff_models = [1.0]
        else:
            coeff_models = np.linspace(coeff_models_deviate, 1-coeff_models_deviate, len(model_name_list))
            coeff_models = list(coeff_models / np.sum(coeff_models))
        return model_name_list, coeff_models, model_device_list

def prompt_encoder(base_prompts: List[str], model, templates_type: str = None, device = device, normalize_first=False):
    if not templates_type:
        templates = no_template
    elif templates_type == 'imagenet':
        templates = imagenet_templates
    elif templates_type == 'avatar':
        templates = avatar_template
    else:
        raise NotImplementedError       
    with torch.no_grad():
        zeroshot_weights = []
        for base_prompt in base_prompts:
            texts = [template.format(base_prompt) for template in templates] #format with class
...
def get_texture_map_from_color(mesh, color, H, W): # Ali: removed default values for H and W
...
def psScreenshot(vertices, faces, axis, angles, save_path, name="mesh", frame_folder="frames", scalars=None,
                 ...
                 cmap=None, sminmax=None, cpos=None, clook=None, save_video=True, save_base=False,
...
if __name__ == '__main__':
    ...
    text_embedding = prompt_encoder(base_prompts=[prompt], model=clip_model, 
                                    templates_type = 'avatar', device = device,
